...
import { isNil, isEmpty, chain } from "lodash";
import { generateAuditResults } from "../utilities.js";
...
function _formatDetails(details, callback) {
  if (isNil(details) || isEmpty(details)) {
    return undefined;
  }
  ...
  if (details.args) {
    details.args.forEach((arg) => {
      switch (arg.type) {
        ...
        case "DURATION":
        case "PERCENTAGE":
          text = text.replace(format("{{%s}}", arg.key), arg.value);
          break;
        case "URL":
          text = text.replace(format("{{%s}}", arg.key), arg.value);
          link = arg.value;
      ...
      if (block && !isEmpty(block)) {
        ...
        if (block.urls) {
          block.urls.forEach((url) => {
            const moreDetails = _formatDetails(url.result, _onUnknownArg);
...
function process(env, task, ackCallback) {
  const gpsiApiKey = env.API_KEY_GPSI;
  const params = {
    url: task.url,
    key: gpsiApiKey,
  };
  debug(`GPSI params: ${JSON.stringify(params)}`);
  ...
  fetch(apiURL)
    ...
    .then((results) => {
      ...
      if (lighthouseResult === undefined) {
        ...
        const res = generateAuditResults(task, rules);
