...
const getChromeTempDataDir = (browser) => {
  ...
  return browser
    .process()
    .spawnargs.reduce((dir, arg) => (arg.startsWith('--user-data-dir=') ? arg.replace('--user-data-dir=', '') : dir), null);
};
...
async function fetch(url, callback) {
  try {
    ...
    const browser = confString && confString.length > 0 ? await puppeteer.launch(JSON.parse(confString)) : await puppeteer.launch();
